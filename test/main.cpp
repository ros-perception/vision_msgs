...
TEST(vision_msgs, CreateAABB2D)
{
  ...
  EXPECT_FLOAT_EQ(bbox.center.x, 2);  // (1 + 3)/2
  EXPECT_FLOAT_EQ(bbox.center.y, 3);  // (2 + 4)/2
...
TEST(vision_msgs, CreateAABB3D)
{
  ...
  EXPECT_FLOAT_EQ(bbox.center.position.x, 2.5);   // (1 + 4)/2
  EXPECT_FLOAT_EQ(bbox.center.position.y, 3.5); // (2 + 5)/2
  EXPECT_FLOAT_EQ(bbox.center.position.z, 4.5);   // (3 + 6)/2
